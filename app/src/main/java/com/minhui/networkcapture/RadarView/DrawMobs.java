...
public class DrawMobs {
    ...
    public void draw(Canvas canvas, float lpX, float lpY, Matrix transformationMatrix, BitmapCache bitmapCache) {
        ...
        for (Mob m : mobList) {
            ...
            Bitmap bitmap = null;
            if (m.getName() != null && view != null) {
                bitmap = bitmapCache.getBitmapFromMemCache(m.getName());
                if (bitmap == null) {
                    try {
                        int resId = view.getResources().getIdentifier(m.getName(), "drawable", view.getContext().getPackageName());
                        if (resId != 0) {
                            Drawable d = ContextCompat.getDrawable(view.getContext(), resId);
                            if (d instanceof BitmapDrawable) {
                                bitmap = ((BitmapDrawable) d).getBitmap();
                                bitmapCache.addBitmapToMemoryCache(m.getName(), bitmap);
                            }
                        }
            ...
            if (bitmap != null) {
                ...
                String label = (m.getName() != null) ? m.getName() : "Mob " + m.getTypeId();
